...
def update(rollouts, sigma_rewards, learning_rate):
    """
...
for learn_rate in [0.02, 0.04, 0.06, 0.08]:
    ...
    for episode in range(EPISODES):
        ...
        update(rollouts, sigma_rewards, learn_rate)

        reward_eval, num_steps = explore(render=render)
        data_point = [episode + 1, reward_eval, num_steps, avg_steps, sigma_rewards]
        data.append(data_point)
        if episode % 5 == 0:
            print('Episode: ', episode + 1, '   Reward: ', reward_eval, '   Alpha: ', learn_rate)
    print(theta)
    end = time.time()
    runtime = end - start
    data.insert(0, [runtime, learn_rate, 0, 0, 0])
    filename = "Bipedal_data_" + str(int(learn_rate * 100)) + ".csv"
    np.savetxt(filename,
               data,
               delimiter=", ",
               fmt='% s')
